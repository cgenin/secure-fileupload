...
public class JpgValidOneFileTest {
    ...
    public void init() throws IOException {
        ...
        uploadedFile.setFileName("test.jpeg");
    ...
    @Test
    public void should_be_ko_if_unknown_extension() {
        uploadedFile.setFileName("tyty.exe");
        when(fileAdapter.readContentFile("toto")).thenReturn(Single.just(jpegBytes));
        when(fileAdapter.write(anyString(), any(byte[].class))).thenReturn(Single.just(true));
        new ValidOneFile(uploadedFile)
                .valid(event, fileAdapter)
                .subscribe(testSubscriber);
        testSubscriber.assertValue(UploadState.invalid_extension);

    }

    @Test
    public void should_be_ko_if_content_multiple_extension() {
        uploadedFile.setFileName("tyty.exe.jpg");
        when(fileAdapter.readContentFile("toto")).thenReturn(Single.just(jpegBytes));
        when(fileAdapter.write(anyString(), any(byte[].class))).thenReturn(Single.just(true));
        new ValidOneFile(uploadedFile)
                .valid(event, fileAdapter)
    ...
    @Test
    public void should_be_ko_if_fail_to_write() {
        when(fileAdapter.readContentFile("toto")).thenReturn(Single.just(jpegBytes));
        when(fileAdapter.write(anyString(), any(byte[].class))).thenReturn(Single.error(new IllegalStateException()));
        new ValidOneFile(uploadedFile)
                .valid(event, fileAdapter)
                .subscribe(testSubscriber);
        testSubscriber.assertValue(UploadState.wrong_sanitization);
