...
public class ExcelValidOneFileTest {


    private UploadedFile uploadedFile;
    private Event event;
    private FileAdapter fileAdapter;
    private byte[] exclBytes;
    ...
    public void init() throws IOException {
        ...
        uploadedFile.setFileName("test.xlsx");
        uploadedFile.setUploadedFileName("toto");
        event = new Event();
        event.setIdApplication("TEST");
        event.setExtensions(Collections.singletonList("EXCEL"));
        fileAdapter = mock(FileAdapter.class);
        testSubscriber = new TestObserver<>();

        exclBytes = Files.readAllBytes(Paths.get("src/test/resources/excel.xlsx"));
    ...
    public void should_be_ok() throws InterruptedException {
        when(fileAdapter.readContentFile("toto")).thenReturn(Single.just(exclBytes));
